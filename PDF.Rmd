...
Looking at the model assumptions, the residuals the gaussian assumption is still not validated (we could'nt do some log transformations) but the others seem validated. Looking at the ncvTest we realize that the p-value is still lower than the 5% treshold but has been considerably improved compared to previous models and the Scale Location plot doesn't really show any special tendencies.
...
We used some of these features in our reduced model, lasso is less selective than our model and used a lot more regressors. Therefore, it will be interesting to see how the two different models predict, and to compare their respective RMSE (see file with full code for details).
