...
```{r prediction 1}
lasso=train(log(SalePrice)~., train, method='glmnet', tuneGrid=expand.grid(alpha=1,lambda=seq(0.01,0.1,length=10)))
lasso
```

```{r}
RMSE_lasso_train=sqrt(mean((fitted(lasso)-log(Ytrain))^2))
RMSE_lasso_test=sqrt(mean((predict(lasso, newdata = test)-log(Ytest))^2))
```


```{r prediction 2}
reduced_model = lm(log(SalePrice) ~ MSZoning + LotArea + OverallQual + OverallCond + YearBuilt +  BsmtQual + BsmtFinSF1 + TotalBsmtSF + CentralAir + GrLivArea + KitchenQual + Fireplaces + GarageQual, data = train)
predict = exp(fitted(reduced_model))
RMSE_lm_train=sqrt(mean((fitted(reduced_model)-log(Ytrain))^2))
RMSE_lm_train_2=sqrt(mean((predict-Ytrain)^2))
RMSE_lm_test=sqrt(mean((predict(reduced_model, newdata = test)-log(Ytest))^2))
print(RMSE_lm_train)
