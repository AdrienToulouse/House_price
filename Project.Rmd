...
The residuals still don't verify the assumptions needed for the validity of our model. As before, the mean seem to be zero, but the others assumptions aren't verified. The Breush-Pagan, Durbin-Watson, and Shapiro-Wlik tests give all of them a p-value that is lower than 0.05. Also, there is still observations that aren't plot on the last plot because they have a leverage of one. So we need to reduce even more the number of variables and to use our work in the first to see which observations lead to this problem.
...
Our model starts with all the following variables selected in the part II: MSZoning, LotArea, OverallQual, OverallCond, YearBuilt, YearRemodAdd, RoofMatl, TotalBsmtSF, CentralAir, GrLivArea, KitchenQual, Fireplaces, GarageQual, BsmtQual, BsmtFinSF1.

**Explain why we deleted some of the variables, use part I**
Furthermore, we noticed that most of the observations have a leverage one because of the variable *RoofMatl*. Indeed, there is only one observation (121) with the category Metal of the variable, and this is the same for the categories Membran, Roll, and ClyTile of this variable. So, we decided to drop it in our reduced model.
...
We are now going to use a Lasso regression to find its reduces the model. In fact, adding the $L^1$ penalization induces sparsity in the estimator. It does variable selection by putting some of the coefficients to zero.
...
We use some of these features in our reduced model, but this isn't the same selection. Therefore, it will be interesting to see how the two different models predict, and to compare their respective RMSE.

## Comparison of models

In order to use the split already done, we need to modify these two dataset as we have done in our study. Thus, we are going to...
