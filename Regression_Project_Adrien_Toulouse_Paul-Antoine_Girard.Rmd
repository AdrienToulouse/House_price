...
reduced_model = lm(LogSalePrice ~ MSZoning + LotArea + OverallQual + OverallCond + YearBuilt + TotalBsmtSF + GrLivArea + Neighborhood + GarageCars + Fireplaces, data = train)
RMSE_lm_test=sqrt(mean((predict(reduced_model, newdata = test)-Ytest)^2))
print(RMSE_lm_test)
```

```{r prediction backward method}
backward = lm(formula = LogSalePrice ~ OverallQual + GrLivArea + Neighborhood + 
    BsmtFinSF1 + GarageCars + OverallCond + TotalBsmtSF + RoofMatl + 
    YearBuilt + LotArea + MSZoning + Condition2 + KitchenQual + 
    SaleCondition + Functional + Condition1 + BsmtExposure + 
    HeatingQC + Fireplaces + BsmtQual + Exterior1st + YearRemodAdd + 
    BldgType + BsmtUnfSF + Heating + GarageQual + LotConfig + 
    CentralAir + LandSlope + BsmtFullBath + Foundation + GarageCond + 
    GarageArea + SaleType + WoodDeckSF + Street + HalfBath + 
    FullBath + GarageYrBlt, data = train)
RMSE_back_test=sqrt(mean((predict(backward, newdata = test)-Ytest)^2))
print(RMSE_back_test)
```

The lasso regression model returns predictions that lead to an RMSE of 0.127 on the test dataset. By comparison, our reduced model has a RMSE of 0.124 on the same dataset. THerefore, we can state that our variable selection improve the accuracy of our log predictions. If we want to have the real predictions, we need to take the exponential of our predictions.
